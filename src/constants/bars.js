...
export const bars = [
    ...
    { catchphrase: '엄마, 나 방송에 나왔어요!', filter: gaknimes.filter((item) => item.tags.includes('시참')) },
    {
        ...
        filter: gaknimes.filter((item) => item.genres.includes('일상') || item.genres.includes('마크 외 게임')),
    },
    { catchphrase: '어르신... 고생하시네요...', filter: gaknimes.filter((item) => item.tags.includes('패널티')) },
    {
        catchphrase: '지금까지 이런 크루는 없었다. 이것은 싸우는 건가 협동하는 걸까. 공각기동대입니다.',
        filter: gaknimes.filter((item) => item.tags.includes('공각기동대')),
    },
